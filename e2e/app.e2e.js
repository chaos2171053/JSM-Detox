...
describe('App', () => {
  ...
  it('可以查看用户的订单', async () => {
    await element(by.id('orders-tab')).tap();
    await expect(
      element(by.id('order-item').withDescendant(by.text('订单状态: 准备中'))),
    ).toBeVisible();
  });
  ...
  it('购买紫盒', async () => {
    ...
    await expect(element(by.id('total-price'))).toHaveText('¥22');
    await element(by.id('order-button')).tap();
    await expect(
      element(by.id('order-item').withDescendant(by.text('紫盒'))),
    ).toExist();
  });
